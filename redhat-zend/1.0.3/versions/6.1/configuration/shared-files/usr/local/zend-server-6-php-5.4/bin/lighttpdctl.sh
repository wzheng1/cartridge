...
start_php_fcgi()
{
    ...
    $ZCE_PREFIX/gui/lighttpd/bin/spawn-fcgi -s $ZCE_PREFIX/tmp/php-fastcgi.socket -f "$ZCE_PREFIX/gui/lighttpd/sbin/php -c $ZCE_PREFIX/gui/lighttpd/etc/php-fcgi.ini" -u $WEB_USER -g $WEB_USER -C 5 -P $ZCE_PREFIX/tmp/php-fcgi.pid
    chmod 660 $ZCE_PREFIX/tmp/php-fastcgi.socket
}
kill_php_fcgi()
{
echo `cat $ZCE_PREFIX/tmp/php-fcgi.pid 2>/dev/null`
    # if the $ZCE_PREFIX/gui/lighttpd/tmp/php-fcgi.pid does not contain a valid PID, kill the strays and rm the file before launching
